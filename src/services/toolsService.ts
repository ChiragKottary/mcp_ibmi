...
export class ToolsService {
    ...
    async getInvoiceDetails(args: any) {
            ...
            content: [
                {
                    ...
                    text: `📄 Invoice Details for ${header.SONUMM || header.invoiceNumber || 'N/A'}:\n\n` +
                        ...
                        `Status: ${statusEmoji[(header.status as keyof typeof statusEmoji)] || '❓'} ${header.status || 'Active'}\n` +
    ...
    async getCustomerInvoices(args: any) {
        ...
        const invoiceList = invoices.slice(0, 10).map(invoice =>
            `📄 Invoice ${invoice.invoiceNumber || 'N/A'} - ${formatCurrency(invoice.invoiceAmount)} | ${invoice.status || 'Unknown'}\n` +
    ...
    async getInvoiceStatistics(args: any) {
        ...
        const topCustomersList = topCustomers.slice(0, 5).map((customer, idx) =>
            `${idx + 1}. ${customer.CUSTOMERNAME?.trim() || customer.customerName || 'Unknown'} (${customer.CUSTOMERNUMBER || customer.customerNumber || 'N/A'}): ${customer.INVOICECOUNT || customer.invoiceCount || 0} invoices, ${formatCurrency(parseFloat(String(customer.TOTALAMOUNT || customer.totalAmount || '0')) || 0)}`
        ).join('\n');
    ...
    async getInvoiceLineItems(args: any) {
    ...
    async getInvoiceHeader(args: any) {
