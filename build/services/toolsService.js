...
export class ToolsService {
    async getOrderDetails(args) {
        ...
        const response = await apiService.getOrderDetails(validatedArgs.externalSystem, validatedArgs.orderNumber, validatedArgs.forUpdate, validatedArgs.lockSource, validatedArgs.includeDeleted);
        if (!response.data) {
            return {
                content: [
                    {
        ...
        // Format order lines
        const orderLinesList = order.orderLines?.slice(0, 10).map((line, idx) => `   ${idx + 1}. ${line.articleText1 || line.articleNo || 'No description'} (${line.articleNo || 'N/A'})\n` +
            ...
            `      Status: ${line.externalLineStatus || 'Active'} | Delivery: ${line.deliveryDate || 'TBD'}`).join('\n\n') || 'No order lines available';
...
• Total Inc VAT: ${formatCurrency(order.totalDiscountedPriceIncVat || 0)}`;
    ...
    async getCustomerInvoices(args) {
        ...
        const response = await apiService.getCustomerInvoices(validatedArgs.companyId, validatedArgs.customerNo, validatedArgs.offset, validatedArgs.limit);
        ...
        // Format invoice list
        const invoicesList = invoices.slice(0, 10).map((invoice, idx) => `   ${idx + 1}. **Invoice #${invoice.invoiceNo}** (ID: ${invoice.id})\n` +
            ...
            `      Project: ${invoice.customerProject?.projectName || 'N/A'} (${invoice.customerProject?.projectNo || 'N/A'})`).join('\n\n') || 'No invoices available';
        // Calculate totals
    ...
    async getCustomerProjectInvoices(args) {
        const validatedArgs = GetCustomerProjectInvoicesSchema.parse(args || {});
        const response = await apiService.getCustomerProjectInvoices(validatedArgs.companyId, validatedArgs.customerNo, validatedArgs.projectNo, validatedArgs.offset, validatedArgs.limit);
        if (!response.data || !response.data.invoiceSummaries) {
            return {
                content: [
                    {
